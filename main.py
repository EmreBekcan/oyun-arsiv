...
# ════════════════════════════════════════════════════════
#  GITHUB YÜKLE DİYALOĞU
# ════════════════════════════════════════════════════════
class GitYukleDialog(ctk.CTkToplevel):
    """Uygulama içinden GitHub'a git push yapma diyaloğu."""

    APP_DIR = os.path.dirname(os.path.abspath(__file__))

    def __init__(self, master):
        super().__init__(master)
        self.title("📤  GitHub'a Güncelleme YüKle")
    ...
    # ── UI ─────────────────────────────────────────────────
    def _olustur(self):
        ctk.CTkLabel(self, text="📤  GitHub Güncelleme YüKleyici",
                     font=("Segoe UI", 16, "bold"),
                     text_color=C["accent"]).pack(pady=(18, 4), padx=20, anchor="w")

        # ── Remote URL ──
        frm_remote = ctk.CTkFrame(self, fg_color=C["sidebar"], corner_radius=10)
        frm_remote.pack(fill="x", padx=16, pady=(6, 4))
        ctk.CTkLabel(frm_remote, text="GitHub Repo URL (remote origin)",
                     font=("Segoe UI", 11, "bold"), text_color=C["text2"]).pack(
                     anchor="w", padx=14, pady=(10, 3))
        self.e_remote = ctk.CTkEntry(
            frm_remote, placeholder_text="https://github.com/EmreBekcan/oyun-arsiv.git",
            font=("Segoe UI", 12), height=36)
        self.e_remote.pack(fill="x", padx=14, pady=(0, 10))
        ctk.CTkButton(frm_remote, text="Kaydet / Güncelle", width=160, height=32,
                      fg_color=C["card"], hover_color=C["accent"],
                      font=("Segoe UI", 11),
                      command=self._remote_kaydet).pack(anchor="e", padx=14, pady=(0, 10))

        # ── Sürüm & Commit ──
        frm_commit = ctk.CTkFrame(self, fg_color=C["sidebar"], corner_radius=10)
        frm_commit.pack(fill="x", padx=16, pady=4)

        ic = ctk.CTkFrame(frm_commit, fg_color="transparent")
        ic.pack(fill="x", padx=14, pady=10)

        # Sol: sürüm
        sol = ctk.CTkFrame(ic, fg_color="transparent")
        sol.pack(side="left", fill="x", expand=True, padx=(0, 10))
        ctk.CTkLabel(sol, text="Yeni Sürüm (tag)",
                     font=("Segoe UI", 11, "bold"), text_color=C["text2"]).pack(anchor="w")
        self.e_versiyon = ctk.CTkEntry(
            sol, placeholder_text="1.1.0",
            font=("Segoe UI", 14, "bold"), height=38)
        self.e_versiyon.pack(fill="x", pady=(4, 0))
        # Mevcut versiyonu doldur
        try:
            self.e_versiyon.insert(0, updater.VERSION)
        except Exception:
            pass

        # Sağ: commit mesajı
        sag = ctk.CTkFrame(ic, fg_color="transparent")
        sag.pack(side="left", fill="x", expand=True)
        ctk.CTkLabel(sag, text="Commit Mesajı",
                     font=("Segoe UI", 11, "bold"), text_color=C["text2"]).pack(anchor="w")
        self.e_mesaj = ctk.CTkEntry(
            sag, placeholder_text="örn: v1.1.0 - Bug fix, yeni özellik",
            font=("Segoe UI", 12), height=38)
        self.e_mesaj.pack(fill="x", pady=(4, 0))

        # ── updater.py'deki VERSION'u da güncelle seçeneği ──
        self.var_surum_guncelle = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(frm_commit,
                        text="updater.py VERSION sabitını otomatik güncelle",
                        variable=self.var_surum_guncelle,
                        font=("Segoe UI", 11), text_color=C["text2"]).pack(
                        anchor="w", padx=14, pady=(0, 10))

        # ── Yükleme butonu ──
        self.btn_yukle = ctk.CTkButton(
            self, text="🚀  GitHub'a YüKle (git push)",
            font=("Segoe UI", 14, "bold"), height=44,
            fg_color="#24292e", hover_color="#444d56",
            command=self._yukle_baslat)
        self.btn_yukle.pack(fill="x", padx=16, pady=(8, 4))

        # ── Terminal çıktısı ──
        ctk.CTkLabel(self, text="Terminal Çıktısı",
                     font=("Segoe UI", 10, "bold"), text_color=C["text2"]).pack(
                     anchor="w", padx=18, pady=(6, 2))
        self.txt_cikti = ctk.CTkTextbox(
            self, font=("Courier New", 10),
            fg_color="#0d0d0d", text_color="#39ff14",
            corner_radius=8)
        self.txt_cikti.pack(fill="both", expand=True, padx=16, pady=(0, 6))
        self.txt_cikti.configure(state="disabled")

        # ── Durum etiketi ──
        self.lbl_durum = ctk.CTkLabel(
            self, text="", font=("Segoe UI", 11),
            text_color=C["text2"])
        self.lbl_durum.pack(pady=(0, 12))

    # ── Yardımcılar ──────────────────────────────────────────
    def _log(self, metin: str, renk: str = "#39ff14"):
        """Terminal kutusuna satır ekler — main thread'den çağrılmali."""
        self.txt_cikti.configure(state="normal")
        self.txt_cikti.insert("end", metin + "\n")
        self.txt_cikti.configure(state="disabled")
        self.txt_cikti.see("end")

    def _durum_guncelle(self):
        """Mevcut git remote'u okuyor ve forma doldur."""
        try:
            sonuc = subprocess.run(
                ["git", "-C", self.APP_DIR, "remote", "get-url", "origin"],
                capture_output=True, text=True, timeout=5)
            url = sonuc.stdout.strip()
            if url:
                self.e_remote.delete(0, "end")
                self.e_remote.insert(0, url)
                self._log(f"✅ Mevcut remote: {url}")
            else:
                self._log("⚠️  Remote henüz ayarlanmamış. URL girerek 'Kaydet' butonuna bas.")
        except Exception as e:
            self._log(f"⚠️  Git kontrol hatası: {e}")

    def _remote_kaydet(self):
        url = self.e_remote.get().strip()
        if not url:
            messagebox.showwarning("Hata", "Remote URL boş olamaz!", parent=self)
            return
        try:
            # Önce mevcut origin var mı?
            var = subprocess.run(
                ["git", "-C", self.APP_DIR, "remote"],
                capture_output=True, text=True, timeout=5)
            if "origin" in var.stdout:
                cmd = ["git", "-C", self.APP_DIR, "remote", "set-url", "origin", url]
                eylem = "güncellendi"
            else:
                cmd = ["git", "-C", self.APP_DIR, "remote", "add", "origin", url]
                eylem = "eklendi"
            subprocess.run(cmd, check=True, timeout=5)
            self._log(f"✅ Remote {eylem}: {url}")
            self.lbl_durum.configure(text=f"✅ Remote {eylem}", text_color=C["success"])
        except subprocess.CalledProcessError as e:
            self._log(f"❌ Remote hata: {e}")

    def _updater_versiyon_guncelle(self, yeni_versiyon: str):
        """updater.py içindeki VERSION sabitini günceller."""
        updater_yol = os.path.join(self.APP_DIR, "updater.py")
        with open(updater_yol, "r", encoding="utf-8") as f:
            icerik = f.read()
        import re
        yeni = re.sub(
            r'^(VERSION\s*=\s*)["\'].+?["\']',
            f'VERSION      = "{yeni_versiyon}"',
            icerik, flags=re.MULTILINE)
        with open(updater_yol, "w", encoding="utf-8") as f:
            f.write(yeni)
        updater.VERSION = yeni_versiyon

    def _cmd_calistir(self, komutlar: list, bitti_cb):
        """
        komutlar: [(etiket, [cmd listesi]), ...]
        Her komutu sırayla çalıştırır, çıktıyı log'a yazar.
        Hata olursa durur. Bitince bitti_cb() çağırır.
        """
        def _run():
            for etiket, cmd in komutlar:
                self.after(0, lambda e=etiket: self._log(f"\n$ {e}"))
                try:
                    ...
                    for satir in proc.stdout:
                        s = satir.rstrip()
                        self.after(0, lambda x=s: self._log(x))
                    proc.wait(timeout=60)
                    if proc.returncode != 0:
                        self.after(0, lambda: self._log(
                            f"❌ Hata! Çıkış kodu: {proc.returncode}", "#ff4444"))
                        self.after(0, lambda: self.lbl_durum.configure(
                            text="❌ Hata oluştu! Terminal çıktısına bakın.",
                            text_color=C["error"]))
                        self.after(0, lambda: self.btn_yukle.configure(
                            state="normal", text="🚀  GitHub'a Yükle (git push)"))
                        self._calisma = False
                        return
                except Exception as e:
                    self.after(0, lambda x=str(e): self._log(f"❌ İstisna: {x}"))
                    self._calisma = False
                    return
            self.after(0, bitti_cb)
            self._calisma = False
        import threading
    ...
    def _yukle_baslat(self):
        if self._calisma:
            return
        versiyon = self.e_versiyon.get().strip().lstrip("v")
        mesaj    = self.e_mesaj.get().strip()
        if not versiyon:
            messagebox.showwarning("Eksik", "Sürüm numarası giriniz!", parent=self)
            return
        if not mesaj:
            mesaj = f"v{versiyon} - güncelleme"

        # updater.py VERSION güncelle
        if self.var_surum_guncelle.get():
            try:
                self._updater_versiyon_guncelle(versiyon)
                self._log(f"✅ updater.py VERSION → {versiyon}")
            except Exception as e:
                self._log(f"⚠️ VERSION güncellenemedi: {e}")

        self._calisma = True
        self.btn_yukle.configure(state="disabled", text="⏳  Yükleniyor...")
        self.lbl_durum.configure(text="⏳  Çalışıyor...", text_color=C["warning"])
        self.txt_cikti.configure(state="normal")
        self.txt_cikti.delete("1.0", "end")
        self.txt_cikti.configure(state="disabled")

        komutlar = [
            ("git add .",     ["git", "add", "-A"]),
            (f"git commit -m \"{mesaj}\"",
             ["git", "commit", "-m", mesaj]),
            (f"git tag v{versiyon}",
             ["git", "tag", f"v{versiyon}"]),
            ("git push origin master --force-if-includes",
             ["git", "push", "origin", "master", "--force-if-includes"]),
            (f"git push origin v{versiyon} (tag)",
             ["git", "push", "origin", f"v{versiyon}"]),
        ]
        self._cmd_calistir(komutlar, self._yukle_bitti)

    def _yukle_bitti(self):
        self._log("\n✅ GitHub'a başarıyla yüklendi!")
        self.btn_yukle.configure(
            state="normal",
            text="🚀  GitHub'a Yükle (git push)")
        self.lbl_durum.configure(
            text=f"✅ v{self.e_versiyon.get().strip()} yüklendi!",
            text_color=C["success"])
        messagebox.showinfo(
            "✅ Başarılı!",
            f"v{self.e_versiyon.get().strip()} GitHub'a yüklendi!\n\n"
            f"GitHub → Releases'tan yeni bir Release oluşturmayı\n"
            f"unutmayın — aksi hâlde otomatik güncelleme çalışmaz.\n\n"
            f"https://github.com/EmreBekcan/oyun-arsiv/releases/new",
            parent=self)
